...
u8 RTC_Init(void)
{
	...
	{	 			
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2019,10,11,15,35,0);  //设置时间	
...
{		 
	...
	{							
		RTC_Get();//更新时间  
		sprintf((char *)current_time_buf,"%d-%d-%d %d:%d:%d",calendar.w_year,calendar.w_month,calendar.w_date,calendar.hour,calendar.min,calendar.sec);
//		printf("RTC Time:%s\r\n",current_time_buf);
