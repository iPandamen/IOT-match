...
/********************************************************
*@brief		：输入热敏电阻两端ADC值，输出热敏电阻值，单位：Ω；
*@param		：uint16_t ADC_Value，热敏电阻两端ADC值，
*@retval	：热敏电阻值，单位：Ω；
*@attention	：ADC分辨率：12位分辨率，2^12=4096
**********************************************************/
uint32_t calculateResValue(uint16_t ADC_Value)
{
	...
	Vrt=(V_REF/(4096-1))*ADC_Value;//热敏电阻两端电压、
//	printf("temperature V %f\r\n",Vrt);
	Rrt= Vrt/((V_REF-Vrt)/Res);//热敏电阻，阻值
...
/********************************************************
*@brief		：输入热敏电阻，单位：Ω，输出温度值，单位：℃
*@param		：uint32_t Res，热敏电阻值，单位：Ω
*@retval	：输出温度值，单位：℃
*@attention	：电阻值范围98.26KΩ~0.3835KΩ，温度范围-20℃~120℃
**********************************************************/
...
int8_t calculateTemperature(uint32_t Rth)
{
	...
	for(i=0;i<140;i++)
	{
		if(i == 139)
		{
			return 127;//超出温度最大测量范围
		}
		else if(i == 0)
		{
			if(Rth > RES_TEMP_Table[0])
			{
				return -128;//超出温度最小测量范围
			}
		}
...
/********************************************************
*@brief		：测算出当前热敏电阻检测到的温度
*@param		：None
*@retval	：输出温度值，有符号，单位：℃
*@attention：度范围-20℃~120℃
**********************************************************/
...
//	adcVale = ADC_GetConversionValue(ADC1);	// 获取 ADC value
	adcVale = ADCRawData[0];
	resTemp = calculateResValue(adcVale);  // 热敏电阻阻值
	temp = calculateTemperature(resTemp);	 // 计算 温度
