...
/******************************************************************************
*	@function	STM32 GPIO输出方向初始化
*	@brief		按照此函数形参的顺序进行宏定义即可
*	@param		uint32_t RCC_APB2Periph		GPIO对应时钟
*               GPIO_TypeDef* GPIOx			所对应的GPIO端口
*               uint16_t GPIO_Pinx			GPIO对应的管脚号
*               GPIOSpeed_TypeDef GPIO_SPEEDGPIO	翻转速度
...
/******************************************************************************
*	@function	void STM32_Flash_Capacity(uint8_t *STMCapa)
*	@brief 	 	读取芯片闪存容量
*	@param      *STMCapa：容量字符显示缓存
*	@retval      None 
******************************************************************************/
void STM32_Flash_Capacity(uint8_t *STMCapa)
{
	...
	if((capa / 1000) != 0)	//千位不为0时显示
		STMCapa[7] = capa / 1000 + 48;	
	else	
		STMCapa[7] = 0x20;
...
/******************************************************************************
*	@function	void STM32_CPUID(uint8_t *IDbuff)  
*	@brief		读取CPUID
*	@param 		存储CPUID缓存（字符串输出）
*	@retval		None 
******************************************************************************/
